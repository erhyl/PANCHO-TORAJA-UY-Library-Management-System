...
namespace Project5LMS
{
    public partial class LoginForm : Form
    {
        ...
        private async void btnSignin_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string email = txtUsername.Text.Trim().ToLowerInvariant();
                ...
                // Check for SQL injection in password only (email is already validated and normalized)
                if (InputValidator.ContainsSqlInjection(password))
                {
                    ShowError("Invalid characters detected in password.", txtPassword);
                    return;
                }
                await Task.Run(() =>
                {
            ...
            if (user == null)
            {
                ...
                try
                {
                    ...
                    using (var conn = dbContext.GetConnection())
                    {
                        ...
                        if (DatabaseSchemaHelper.CheckColumnExists(conn, "Users", "Status"))
                        {
                            string checkStatusQuery = "SELECT COALESCE(Status, 'Active') as Status FROM Users WHERE Email = @email LIMIT 1";
                            using (var cmd = new MySqlCommand(checkStatusQuery, conn))
                            {
                                cmd.Parameters.AddWithValue("@email", email);
                                object statusResult = cmd.ExecuteScalar();
                                if (statusResult != null && statusResult.ToString() == "Suspended")
                                {
                                    MessageBox.Show("Your account has been suspended. Please contact an administrator for assistance.",
                                        "Account Suspended", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                    AuditLogger.LogSecurity("Suspended User Login Attempt", $"Email: {email}", "Blocked");
                                    return;
                                }
                            }
                        }
                ...
                {
                    // If we can't check status, continue with normal error handling
                }
