...
namespace Project5LMS.Forms.LibraryStaff.Members
{
    public partial class StaffMembersForm : Form
    {
        ...
        private void SetupDataGridView()
        {
            ...
            // Staff role restriction: Staff can only add and view members, cannot edit or delete
            // Edit and Delete buttons are only available to Admin users
            ...
            // Only Admin can delete members - Staff role restriction
            if (CurrentUser.Role != null && CurrentUser.Role.Equals("Admin", StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void EditMember(int memberId)
        {
            try
            {
                // Staff can edit member info but cannot change status to Suspended
                // The AddMemberForm should handle this restriction internally
        ...
        {
            // Staff role restriction: Only Admin can delete members
            if (CurrentUser.Role == null || !CurrentUser.Role.Equals("Admin", StringComparison.OrdinalIgnoreCase))
            {
                MessageBox.Show("Only administrators can delete members.", "Access Denied", 
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                AuditLogger.LogAccessControl("Delete Member attempt denied", 
