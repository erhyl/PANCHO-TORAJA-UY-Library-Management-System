...
using Project5LMS.Forms.LibraryStaff.Settings;
namespace Project5LMS.Forms.LibraryStaff.Dashboard
{
    public partial class StaffDashboardForm : Form
    {
        ...
        private void btnChangePassword_Click(object sender, EventArgs e)
        {
            try
            {
                using (var changePasswordForm = new StaffChangePasswordForm())
                {
            ...
            {
                MessageBox.Show($"Error opening change password form: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void LoadMetrics()
        {
            try
            {
                ...
                // Show books added this month as change indicator
                if (booksThisMonth > 0)
                {
                    lblBooksCatalogChange.Text = $"+{booksThisMonth} this month";
                    lblBooksCatalogChange.ForeColor = Color.FromArgb(40, 167, 69);
                }
                else
                {
                    lblBooksCatalogChange.Text = "0 this month";
                    lblBooksCatalogChange.ForeColor = Color.FromArgb(128, 128, 128);
                }
                
                // Today's Borrowings
                int borrowedToday = _dashboardService.GetBorrowedToday();
                lblActiveLoansValue.Text = borrowedToday.ToString("N0");
                int activeLoans = _dashboardService.GetActiveBorrowings();
                if (activeLoans > 0)
                {
                    lblActiveLoansChange.Text = $"{activeLoans} active";
                    lblActiveLoansChange.ForeColor = Color.FromArgb(40, 167, 69);
                }
                else
                {
                    lblActiveLoansChange.Text = "0 active";
                    lblActiveLoansChange.ForeColor = Color.FromArgb(128, 128, 128);
                }
                
                // Today's Returns
                int returnedToday = _dashboardService.GetReturnedToday();
                lblTodayReturnsValue.Text = returnedToday.ToString("N0");
                if (returnedToday > 0)
                {
                    lblTodayReturnsChange.Text = $"{returnedToday} today";
                    lblTodayReturnsChange.ForeColor = Color.FromArgb(40, 167, 69);
                }
                else
                {
                    lblTodayReturnsChange.Text = "0 today";
                    lblTodayReturnsChange.ForeColor = Color.FromArgb(128, 128, 128);
                }
                ...
                lblReservationsValue.Text = totalReservations.ToString("N0");
                if (pendingReservations > 0)
                {
                    lblReservationsChange.Text = $"{pendingReservations} pending";
                    lblReservationsChange.ForeColor = Color.FromArgb(255, 193, 7); // Amber for attention
                }
                else
                {
                    lblReservationsChange.Text = "0 pending";
                    lblReservationsChange.ForeColor = Color.FromArgb(128, 128, 128);
                }
                
                // Pending Fines
                decimal pendingFines = _dashboardService.GetPendingFines();
                lblPendingFinesValue.Text = Project5LMS.Helpers.IDFormatter.FormatCurrency((decimal)pendingFines, 0);
                // Show fines collected today
                decimal finesCollectedToday = _dashboardService.GetFinesCollectedToday();
                if (finesCollectedToday > 0)
                {
                    lblPendingFinesChange.Text = $"{Project5LMS.Helpers.IDFormatter.FormatCurrency((decimal)finesCollectedToday, 0)} collected today";
                    lblPendingFinesChange.ForeColor = Color.FromArgb(40, 167, 69);
                }
                else
                {
                    lblPendingFinesChange.Text = "₱0 collected today";
                    lblPendingFinesChange.ForeColor = Color.FromArgb(128, 128, 128);
                }
                
                // Overdue count (shown in overdue books list below, but we can also show it)
                int overdueCount = _dashboardService.GetOverdueBooks();
            }
            catch (Exception ex)
            {
