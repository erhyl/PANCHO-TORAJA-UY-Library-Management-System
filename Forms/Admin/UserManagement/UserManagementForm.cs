...
namespace Project5LMS.Forms.Admin.UserManagement
{
    public partial class UserManagementForm : Form
    {
        ...
        private void EnsureStatusColumnExists()
        {
            try
            {
                ...
                using (var conn = dbContext.GetConnection())
                {
                    ...
                    using (var cmd = new MySqlCommand(checkColumnQuery, conn))
                    {
                        int columnExists = Convert.ToInt32(cmd.ExecuteScalar());
                        if (columnExists == 0)
                        {
                            // Create Status column
                            string alterTableQuery = "ALTER TABLE Users ADD COLUMN Status VARCHAR(50) DEFAULT 'Active'";
                            using (var alterCmd = new MySqlCommand(alterTableQuery, conn))
                            {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error ensuring Status column exists: {ex.Message}");
            }
        }
        ...
        private Dictionary<int, string> userStatusMap = new Dictionary<int, string>();
            ...
            try
            {
                ...
                foreach (DataRow row in result.Rows)
                {
                    ...
                    string status = row.Table.Columns.Contains("Status") 
                        ...
                        : "Active";
                    userStatusMap[user.UserID] = status;
                }
        ...
        private Panel CreateUserCard(User user)
        {
            ...
            string status = userStatusMap.ContainsKey(user.UserID) ? userStatusMap[user.UserID] : "Active";
            bool isSuspended = status == "Suspended";
            Label lblStatus = new Label
            {
                Text = status,
                Font = new Font("Segoe UI", 9, FontStyle.Regular),
                ForeColor = isSuspended ? Color.FromArgb(220, 53, 69) : Color.FromArgb(40, 167, 69),
                BackColor = isSuspended ? Color.FromArgb(248, 215, 218) : Color.FromArgb(212, 237, 218),
                AutoSize = true,
                Padding = new Padding(8, 4, 8, 4),
                Location = new Point(tagX, tagY)
            };
            ...
            string buttonStatus = userStatusMap.ContainsKey(user.UserID) ? userStatusMap[user.UserID] : "Active";
            bool userIsSuspendedForButton = buttonStatus == "Suspended";
            Button btnSuspend = new Button
            {
                Text = userIsSuspendedForButton ? "Unsuspend" : "Suspend",
                Font = new Font("Segoe UI", 9, FontStyle.Regular),
                ForeColor = userIsSuspendedForButton ? Color.FromArgb(40, 167, 69) : Color.FromArgb(220, 53, 69),
                BackColor = userIsSuspendedForButton ? Color.FromArgb(212, 237, 218) : Color.FromArgb(248, 215, 218),
                FlatStyle = FlatStyle.Flat,
                FlatAppearance = { BorderSize = 0 },
            ...
            btnSuspend.Click += (s, e) => ToggleSuspendUser(user);
        ...
        {
            var result = MessageBox.Show(
                $"Reset password for {user.FirstName} {user.LastName}❓\n\nA temporary password will be generated and the user will need to change it on next login.",
                "Reset Password",
                MessageBoxButtons.YesNo,
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    AuditLogger.LogDataModification("Password Reset",
                        $"UserID: {user.UserID}, Email: {user.Email}, Reset by: {CurrentUser.Email}",
                        "Success");
                ...
                catch (Exception ex)
                {
                    ...
                    AuditLogger.LogDataModification("Password Reset Failed",
                        $"UserID: {user.UserID}, Error: {ex.Message}",
                        "Failed");
                }
            }
        }
        ...
        private void ToggleSuspendUser(User user)
        {
            string currentStatus = userStatusMap.ContainsKey(user.UserID) ? userStatusMap[user.UserID] : "Active";
            bool userIsSuspended = currentStatus == "Suspended";
            string action = userIsSuspended ? "unsuspend" : "suspend";
            string actionTitle = userIsSuspended ? "Unsuspend User" : "Suspend User";
            string newStatus = userIsSuspended ? "Active" : "Suspended";
            
            var result = MessageBox.Show(
                $"{actionTitle.Substring(0, 1).ToUpper() + actionTitle.Substring(1)} {user.FirstName} {user.LastName}❓\n\n" +
                $"This will {(userIsSuspended ? "restore" : "revoke")} the user's access to the system.",
                actionTitle,
                MessageBoxButtons.YesNo,
                MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    AuditLogger.LogDataModification(actionTitle,
                        $"UserID: {user.UserID}, Email: {user.Email}, Status changed to: {newStatus}, Changed by: {CurrentUser.Email}",
                        "Success");
                    
                    MessageBox.Show($"User {action}ed successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadUsers();
                    LoadMetrics();
                }
                catch (MySqlException ex)
                {
                    if (ex.Message.Contains("Unknown column 'Status'"))
                    {
                        MessageBox.Show("Status column not found in Users table. The column will be created automatically. Please try again.",
                ...
                {
                    MessageBox.Show($"Error {action}ing user: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    AuditLogger.LogDataModification($"{actionTitle} Failed",
                        $"UserID: {user.UserID}, Error: {ex.Message}",
                        "Failed");
                }
            }
