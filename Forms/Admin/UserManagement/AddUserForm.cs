...
namespace Project5LMS.Forms.Admin.UserManagement
{
    public partial class AddUserForm : Form
    {
        ...
        {
            cmbRole.Items.Clear();
            cmbRole.Items.AddRange(new string[] { "Admin", "LibraryStaff" });
                    ...
                    using (var cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                string role = reader["Role"] != DBNull.Value ? reader["Role"].ToString() : "LibraryStaff";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string role = cmbRole.SelectedItem != null ? cmbRole.SelectedItem.ToString() : "LibraryStaff";
            try
            {
                if (_isEditMode)
                {
        ...
        private bool EmailExists(string email)
        {
            try
            {
                using (var conn = _dbContext.GetConnection())
                {
                    ...
                    string query = "SELECT COUNT(*) FROM Users WHERE LOWER(TRIM(Email)) = @Email";
                    using (var cmd = new MySqlCommand(query, conn))
                    {
