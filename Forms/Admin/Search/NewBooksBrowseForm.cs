...
{
    public partial class NewBooksBrowseForm : Form
    {
        ...
        public NewBooksBrowseForm()
        {
            InitializeComponent();
        }
        private void NewBooksBrowseForm_Load(object sender, EventArgs e)
        {
            SetupDataGridView();
            LoadNewBooks();
        }
        ...
        private void LoadNewBooks()
        {
            try
            {
                // Default to last 30 days for new arrivals
                DateTime endDate = DateTime.Now;
                DateTime startDate = endDate.AddDays(-30);
                
                var newBooks = BookService.GetNewArrivals(50, startDate, endDate);
                ...
                else
                {
                    ...
                    MessageBox.Show("No new books found in the last 30 days.", "No Results", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error loading new books: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
