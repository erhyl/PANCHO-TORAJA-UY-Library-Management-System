    ...
    public partial class AdminDashboardForm : Form
    {
        ...
        private void LoadMetrics()
        {
            try
            {
                ...
                // Calculate overdue books from last week (7 days ago)
                // For now, show current count - a more accurate comparison would require historical data
                // This is a reasonable approximation
                lblOverdueChange.Text = $"{overdueBooks} overdue";
                lblOverdueChange.ForeColor = overdueBooks > 0 ? Color.FromArgb(220, 53, 69) : Color.FromArgb(40, 167, 69);
                decimal pendingFines = _dashboardService.GetPendingFines();
                lblFinesValue.Text = Project5LMS.Helpers.IDFormatter.FormatCurrency((decimal)pendingFines, 0);
                decimal finesCollectedToday = _dashboardService.GetFinesCollectedToday();
                lblFinesChange.Text = $"{Project5LMS.Helpers.IDFormatter.FormatCurrency((decimal)finesCollectedToday, 0)} collected today";
