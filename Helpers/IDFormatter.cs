...
namespace Project5LMS.Helpers
{
    public static class IDFormatter
    {
        ...
        public static string FormatCurrency(decimal amount, int decimals = 2)
        {
            return $"₱{amount.ToString($"N{decimals}")}";
        }
        public static string FormatCurrency(double amount, int decimals = 2)
        {
            return FormatCurrency((decimal)amount, decimals);
        }
        public static string FormatCurrency(string amountText, int decimals = 2)
        {
        ...
        public static decimal ParseCurrency(string currencyText)
        {
            if (string.IsNullOrWhiteSpace(currencyText))
                return 0;
            // Remove currency symbols and spaces
            string clean = currencyText.Trim()
                .Replace("₱", "")
                .Replace("Php", "")
                .Replace("PHP", "")
                .Replace("$", "")
                .Replace(",", "")
                .Trim();
            if (decimal.TryParse(clean, out decimal amount))
                return amount;
            return 0;
        }
        #endregion
