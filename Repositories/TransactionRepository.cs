    ...
    public class TransactionRepository : ITransactionRepository
    {
        ...
        public bool Update(CirculationRecord transaction)
        {
            try
            {
                ...
                {
                    conn.Open();
                    // Check if RenewalCount column exists
                    bool hasRenewalCount = DatabaseSchemaHelper.CheckColumnExists(conn, "Transactions", "RenewalCount");
                    
                    // Build UPDATE query with optional RenewalCount column
                    ...
                    if (hasRenewalCount)
                    {
                        setClauses.Add("RenewalCount=@RenewalCount");
                    }
                    
                    string query = $@"UPDATE Transactions SET {string.Join(", ", setClauses)}
