        ...
        public Member GetById(int memberId)
        {
            try
            {
                using (var conn = _dbContext.GetConnection())
                {
                    ...
                    var columns = MemberColumnSchema.GetMemberColumns(conn);
                    var selectColumns = MemberColumnSchema.BuildSelectColumns(columns);
        ...
        public IEnumerable<Member> Search(string searchTerm)
        {
            ...
            try
            {
                using (var conn = _dbContext.GetConnection())
                {
                    ...
                    // Check if MemberCardNumber column exists
                    bool hasMemberCardNumber = DatabaseSchemaHelper.CheckColumnExists(conn, "Members", "MemberCardNumber");
                    
                    // Try to parse member ID from formatted strings like "MEM-000005" or "M5"
                    ...
                    // Get all column information to build complete SELECT
                    var columns = MemberColumnSchema.GetMemberColumns(conn);
                    var selectColumns = MemberColumnSchema.BuildSelectColumns(columns);
                    ...
                    using (var cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        if (parsedMemberId > 0)
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine($"MemberRepository.Search: Parsed member ID '{searchTerm}' -> {parsedMemberId}");
                        }
                        ...
                        using (var adapter = new MySqlDataAdapter(cmd))
                        {
                            DataTable dt = new DataTable();
                            adapter.Fill(dt);
                            System.Diagnostics.Debug.WriteLine($"MemberRepository.Search: Found {dt.Rows.Count} members for search term '{searchTerm}'");
                            foreach (DataRow row in dt.Rows)
                            {
                                members.Add(MapDataRowToMember(row));
                            }
