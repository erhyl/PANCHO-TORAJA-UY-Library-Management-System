...
namespace Project5LMS.Repositories
{
    public class BookCopyRepository : IBookCopyRepository
    {
        ...
        private void EnsureBookCopiesTableExists(MySqlConnection conn)
        {
            try
            {
                DatabaseSchemaHelper.CreateBookCopiesTableIfNotExists(conn);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error ensuring BookCopies table exists: {ex.Message}");
                throw new Exception("Failed to create BookCopies table. Please check database permissions.", ex);
            }
        }
        ...
        public IEnumerable<BookCopy> GetByBookId(int bookId)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    // Check if table exists, if not return empty list (table will be created on first Add)
                    if (!DatabaseSchemaHelper.CheckTableExists(conn, "BookCopies"))
                    {
                        return copies; // Return empty list if table doesn't exist yet
                    }
        ...
        public bool Add(BookCopy copy)
        {
            try
            {
                using (var conn = _dbContext.GetConnection())
                {
                    ...
                    EnsureBookCopiesTableExists(conn);
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception($"Failed to add copy: {ex.Message}", ex);
            }
        }
