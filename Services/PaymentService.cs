    ...
    public class PaymentService : IPaymentService
    {
        ...
        public bool ProcessPayment(FinePayment payment)
        {
            try
            {
                ...
                {
                    // Validate TransactionID exists in Transactions table before using it
                    // If it doesn't exist, set to NULL to avoid foreign key constraint violation
                    int? validTransactionID = null;
                    if (payment.TransactionID > 0)
                    {
                        string checkTransactionQuery = "SELECT COUNT(*) FROM Transactions WHERE TransactionID = @TransactionID";
                        using (var checkCmd = new MySqlCommand(checkTransactionQuery, conn, trans))
                        {
                            ...
                            if (count > 0)
                            {
                                validTransactionID = payment.TransactionID;
                            }
                            // If count is 0, validTransactionID remains null
                        }
                    }
                    ...
                    {
                        // Use validTransactionID if it exists, otherwise NULL
                        if (validTransactionID.HasValue)
                        {
                            cmd.Parameters.AddWithValue("@TransactionID", validTransactionID.Value);
                        }
                        else
                        {
                    ...
                    // Only update transaction if we have a valid TransactionID
                    if (validTransactionID.HasValue)
                    {
                        UpdateTransactionAfterPayment(conn, trans, validTransactionID.Value, payment.AmountPaid);
                    }
                });
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                return _dbContext.ExecuteInTransaction((conn, trans) =>
                {
                    ...
                    if (adjustment.TransactionID > 0)
                    {
                        ...
                        using (var checkCmd = new MySqlCommand(checkTransactionQuery, conn, trans))
                        {
                            checkCmd.Parameters.AddWithValue("@TransactionID", adjustment.TransactionID);
                            object result = checkCmd.ExecuteScalar();
                            int count = result != null && result != DBNull.Value ? Convert.ToInt32(result) : 0;
                            if (count > 0)
                            {
                                validTransactionID = adjustment.TransactionID;
                            }
                            // If count is 0, validTransactionID remains null
                        }
                    }
                    ...
                    if (validTransactionID.HasValue)
                    {
                        UpdateTransactionAfterAdjustment(conn, trans, validTransactionID.Value, adjustment.AdjustedAmount);
                    }
                });
            }
            catch (Exception ex)
        ...
        private FinePayment MapDataRowToPayment(DataRow row)
        {
            return new FinePayment
            {
                ...
                TransactionID = row["TransactionID"] != DBNull.Value ? Convert.ToInt32(row["TransactionID"]) : 0,
