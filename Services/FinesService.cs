...
namespace Project5LMS.Services
{
    public class FinesService : IFinesService
    {
        ...
        public decimal CalculateFine(int transactionId)
        {
            ...
            // Get member type to determine fine rate and max cap
            var memberRepository = DependencyInjection.GetRequiredService<IMemberRepository>();
            var member = memberRepository.GetById(transaction.MemberID);
            if (member != null)
            {
                var privileges = MemberTypePrivileges.GetDefaultPrivileges(member.Type);
                decimal fineRatePerDay = privileges.FineRatePerDay;
                decimal maxFineCap = privileges.MaxFineCap;
                
                // Calculate fine: Days Overdue × Daily Rate (up to maximum)
                decimal calculatedFine = daysOverdue * fineRatePerDay;
                decimal fine = Math.Min(calculatedFine, maxFineCap);
                
                return Math.Round(fine, 2, MidpointRounding.AwayFromZero);
            }
            
            // Fallback to strategy if member not found
            return _fineStrategy.CalculateFine(daysOverdue);
        }
        
        public decimal CalculateFine(int transactionId, int memberId)
        {
            var transaction = _transactionRepository.GetById(transactionId);
            if (transaction == null || transaction.ReturnDate.HasValue)
                return 0m;
